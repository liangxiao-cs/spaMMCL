...
class Encoder(Module):
    def __init__(self, in_features, out_features, graph_neigh, act=F.relu):
        ...
        self.gene2_pre = nn.Sequential(nn.Linear(self.in_features, projection_dims_1), nn.ELU())
        ...
        # img_encoder
        self.liner_feat2 = nn.Sequential(nn.Linear(gcn_hid_1, img_out_features), nn.ELU())
    ...
    def encoding_mask_noise(self, x, feat2_pre, if_mask, mask_rate=0.3):
        if if_mask:
            out_x = x.clone()
            num_nodes = x.shape[0]
            ...
            out_x[token_nodes] = feat2_pre[token_nodes]
            return out_x, mask_nodes, keep_nodes
        else:
            return x, 'mask_nodes', 'keep_nodes'

    def forward(self, feat, feat_a, adj, feat2_pre, if_mask=True, a=0.1):
        # mask
        feat = self.gene_pre(feat)
        feat2_pre = self.gene2_pre(feat2_pre)
        feat = self.projector(feat)
        feat2_pre = self.projector(feat2_pre)
        feat, mask_nodes, keep_nodes = self.encoding_mask_noise(feat, feat2_pre, if_mask, self.mask_rate)

        # xg and xi
        xg = self.gc_gene_1(feat, adj)
        xg = self.gc_gene_2(xg, adj)
        z_gene = xg
        xi = self.gc_gene_1(feat2_pre, adj)
        xi = self.liner_feat2(xi)
        z_gene2 = xi
        z_fusion = torch.cat((z_gene, z_gene2), dim=1)
        ...
        return z, z_gene, z_gene2, h_cross, ret, mask_nodes, keep_nodes, h_gene
...
class MML():
    def __init__(self,
        ...
        # 5 feat_noise
        if 'feat_noise' not in self.adata.obsm.keys():
            self.adata.obsm['feat_noise'] = self.adata.obsm['feat'].copy() + np.random.normal(0, 1, self.adata.obsm['feat'].copy().shape)
        ...
        self.image_features = torch.FloatTensor(self.adata.obsm['feat_noise'].copy()).to(self.device)
    ...
    def train(self):
        ...
        for _ in tqdm(range(self.epochs)):
            self.model.train()
            z, z_gene, z_gene2, h_cross, ret, mask_nodes, keep_nodes, h_gene = self.model(
                self.features,
                self.features_a,
                self.adj_norm,
                self.image_features,
